...
def generate_equity_research_pdf(ticker: str, company_name: str, task_outputs: list, section_names: list, output_path: str):
    """
    ...
    if not pandoc or not xelatex:
        raise RuntimeError("pandoc and xelatex required for PDF generation")
...
def generate_hedge_fund_memo_pdf(ticker: str, company_name: str, task_outputs: list, section_names: list, output_path: str):
    """
...
def governance_data_tool(symbol: str) -> str:
    ...
    filings_resp = _get_json(f"{base_v3}/sec_filings/{symbol}", {"limit": 50, "apikey": api_key})
    filings_data = filings_resp.get("data") if filings_resp.get("ok") else None
...
def business_profile_tool(symbol: str) -> str:
    ...
    ratios_resp = _get_json(f"{base_v3}/ratios-ttm/{symbol}", {"apikey": api_key})
    ...
    classification_hints = {
        "purchase_frequency_hint": _get_purchase_frequency_hint(sector, industry),
        "recession_sensitivity_hint": _get_recession_sensitivity_hint(sector, industry),
    }
...
def _get_purchase_frequency_hint(sector: str, industry: str) -> str:
    """Provide classification hint based on sector/industry."""
...
def _get_recession_sensitivity_hint(sector: str, industry: str) -> str:
...
def build_crew(scrape_tool):
    ...
    sentiment_analyst = Agent(
        ...
        tools=[price_sentiment_data_tool, fmp_news_tool, scrape_tool],
        verbose=True,
        allow_delegation=False,
    )
    ...
    strategist = Agent(
        ...
        goal="Identify Moat Strength and Growth Sources, consistent with the company's phase.",
        backstory="Expert in competitive strategy. You analyze pricing power, network effects, switching costs, and lifecycle stages.",
        tools=[investment_data_tool, fmp_news_tool, scrape_tool],
        verbose=True,
        allow_delegation=False,
    )
    ...
    governance_expert = Agent(
        ...
        tools=[governance_data_tool, fmp_news_tool, scrape_tool],
        verbose=True,
        allow_delegation=False,
    )
    ...
    business_profile_analyst = Agent(
        role="Business Profile Analyst",
        goal="Complete a structured business analysis table answering fundamental questions about how the company makes money, its products, customers, geography, purchase patterns, pricing power, and recession sensitivity.",
        backstory="You are a business model analyst who distills complex companies into clear, structured profiles. You classify businesses based on revenue patterns, competitive position, and economic sensitivity using evidence from financial data and company descriptions.",
    ...
    * [Reason 3 + link (or "Limited recent coverage.")]
    ...
    | **Evidence** | • Operating Income: $[X]M ([improving/worsening/positive/negative]) <br> • Revenue Growth: [X]% <br> • Capital Returns: [Yes/No with specifics] |
    ...
    task_business_profile = Task(
        description=r"""
    You are completing a Business Analysis Table for {ticker}.

    You MUST:
    1) Call business_profile_tool({ticker}) to get company profile, product segments, geographic segments, margin data, and classification hints.
    2) Use the company description, product segments, geographic segments, and financial metrics from the tool output to answer the questions.
    3) Answer ALL 7 questions in the template below.

    CLASSIFICATION RULES:

    **Purchase Frequency** (choose ONE):
    - "Every Few Years" = Durable goods (cars, appliances, furniture), major capital purchases, real estate
    - "Yearly" = Apparel, electronics accessories, annual subscriptions, B2B contracts renewed annually
    - "Recurring" = Monthly/quarterly subscriptions, consumables, utilities, SaaS, insurance premiums

    **Pricing Power** (choose ONE):
    - "Price taker" = Commodity business, no differentiation, gross margins typically <25%
    - "Would lose customers" = Competitive market with some differentiation, gross margins 25-45%
    - "Can increase prices" = Strong brand/moat, high switching costs, gross margins typically >45%, demonstrated history of price increases

    **Recession Sensitivity** (choose ONE):
    - "Highly Cyclical" = Discretionary spending (luxury, travel, autos), revenue drops significantly in recessions
    - "Normal" = Moderate sensitivity, some demand elasticity but not extreme
    - "Recession-Proof" = Essential goods/services (healthcare, utilities, staples), demand stable regardless of economy

    Use the classification_hints from the tool as guidance, but verify with actual data.

    OUTPUT ONLY THE TEMPLATE BELOW:

    # 📋 Business Profile Analysis: [Company Name] ([Ticker])

    ## Business Model Overview

    | Question | Answer |
    | :--- | :--- |
    | **How does the company make money?** | [2-3 sentences explaining primary revenue streams and business model] |
    | **What are its key products/services?** | [Bullet list of main products/services with approximate revenue contribution if available] |
    | **Who are its customers?** | [Customer segments: B2B/B2C/B2G, enterprise/SMB/consumer, specific industries served] |
    | **Where does the company operate?** | [Geographic breakdown with % of revenue if available, e.g., "US (60%), Europe (25%), Asia (15%)"] |

    ## Business Characteristics

    | Characteristic | Classification | Rationale |
    | :--- | :--- | :--- |
    | **Purchase Frequency** | Every Few Years / Yearly / Recurring | [1 sentence explanation based on product type] |
    | **Pricing Power** | Price taker / Would lose customers / Can increase prices | [1 sentence citing gross margin and competitive position] |
    | **Recession Sensitivity** | Highly Cyclical / Normal / Recession-Proof | [1 sentence based on sector and demand elasticity] |
    ...
    ## 🔗 Sources
    - [Company website or IR page]
    - [Any additional sources used]

    STOP HERE - DO NOT ADD ANY ADDITIONAL SECTIONS
    """,
        expected_output="ONLY the filled Business Profile template with all 7 questions answered.",
        agent=business_profile_analyst,
    ...
    You MUST:
    1) Use the search tool to find 5–8 high-signal sources (company IR, investor presentation, credible industry analysis, competitor pages).
    2) Select the best 2–4 sources and use ScrapeWebsiteTool to extract specific evidence.
    3) Include a short "Sources" section at the end listing 4–8 URLs used.
    ...
    2) Use ScrapeWebsiteTool on the most relevant 1–3 links to extract supporting details.
    ...
    - Identify 3–6 key risks (competition, disruption, concentration, regulation, cyclicality, execution).""",
    ...
    return Crew(
        agents=[sentiment_analyst, phase_classifier, business_profile_analyst, strategist, governance_expert, quant_auditor],
        tasks=[task_price_sentiment, task_business_phase, task_business_profile, task_business_moat, task_management_risk, task_quant_valuation],
...
def main():
    ...
    scrape_tool = build_tools()
    investment_crew = build_crew(scrape_tool)

    parser = argparse.ArgumentParser(description="CrewAI Investment Analyzer (FMP API)")
    parser.add_argument("--ticker", type=str, default=None, help="Ticker symbol, e.g. AAPL")
    parser.add_argument("--no-pdf", action="store_true", help="Skip PDF generation")
    args = parser.parse_args()

    ticker_to_analyze = (args.ticker or input("Enter a ticker symbol: ")).strip().upper()
    ...
    section_names = ["PRICE & SENTIMENT", "BUSINESS PHASE", "BUSINESS PROFILE", "BUSINESS & MOAT", "MANAGEMENT & RISK", "QUANT & VALUATION"]
    ...
    equity_pdf = f"{ticker_to_analyze}_equity_research.pdf"
    memo_pdf = f"{ticker_to_analyze}_investment_memo.pdf"

    # Extract company name from the business profile section if available
    company_name = ticker_to_analyze
    for i, task_output in enumerate(result.tasks_output):
        if section_names[i] == "BUSINESS PROFILE" or section_names[i] == "BUSINESS PHASE":
            # Try to extract company name from the output
            lines = task_output.raw.split('\n')
            for line in lines:
                if 'Analysis:' in line and '(' in line:
                    # Extract "Company Name (TICKER)" pattern
                    try:
                        ...
                        pass
            if company_name != ticker_to_analyze:
                break
